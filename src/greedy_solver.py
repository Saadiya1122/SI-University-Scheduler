...
def get_blocks(start, duration, interval_minutes=30, teaching_end_minutes=17 * 60):
...
def can_schedule(
    ...
    blocks = get_blocks(start,class_item["duration_hours"],interval_minutes,teaching_end_minutes)

    if not blocks:
        return False

    end = time_to_minutes(start) + class_item["duration_hours"] * 60
...
def get_time_load(schedule, day, start, duration, interval_minutes=30):
    blocks = get_blocks(start,duration,interval_minutes,24 * 60)
            ...
            for day in ordered_days:
                ...
                for start in ordered_times:
                    same_campus = [
                        room
                        for room in rooms
                        if room.get("campus_id") == class_item["campus_id"]
                    ]
                    ...
                    candidates = (
                        sorted(same_campus, key=lambda x: x["capacity"])
                        + sorted(other_rooms, key=lambda x: x["capacity"])
                    )
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    print("Teaching days:", ", ".join(config["teaching_days"]))
    print("Teaching window:",config["teaching_start"],"-",config["teaching_end"])
    print("Scheduling interval:",config["scheduling_interval_minutes"],"minutes")
    ...
    for quarter in sorted(set(item["quarter_id"] for item in classes)):
        ...
        schedule, unscheduled = schedule_quarter(quarter_classes,rooms,config=config)

        waste = sum(item["capacity"] - item["student_count"] for item in schedule)

        print(quarter, "->", len(schedule), "sessions |", len(unscheduled), "failed classes | waste:", waste)